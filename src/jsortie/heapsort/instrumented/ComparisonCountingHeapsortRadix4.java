...
public class ComparisonCountingHeapsortRadix4 
  extends ComparisonCountingHeapsort {
  public ComparisonCountingHeapsortRadix4() {
    radix = 4;
  }
  public void constructHeap
    ...
    for (int k = start + (stop-start-radix-1)/radix; k>=start ; --k) {
      ...
      do {
        if (j+3<stop) {
          constructionComparisonCount +=3;
          int j1 =  j + (vArray[j]<vArray[j+1] ? 1 : 0);
          int j2 =  j + (vArray[j+2]<vArray[j+3] ? 3 : 2);
          j  = vArray[j1] < vArray[j2] ? j2 : j1;
        }
        else if (j+2<stop) {
