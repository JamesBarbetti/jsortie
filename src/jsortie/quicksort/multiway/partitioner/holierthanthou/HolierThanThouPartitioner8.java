...
public class HolierThanThouPartitioner8 
  ...
  public int[] multiPartitionRange
    ...
    if (pivotIndices.length < desiredPivotCount ) {
      return MultiPivotUtils.fakePartitions
             ( vArray, start, stop
             , pivotIndices, desiredPivotCount );
    }	  
    ...
    for (int scan=b8+1; scan<stop; ++scan) {
      int v = vArray[scan];
      if ( v < v4 ) {
        if ( v < v2 ) {
          if ( v < v1 ) {
    ...
    return new int[] 
      ...
      , b4+1,  b5, b5+1, b6, b6+1, b7, b7+1, b8
      , b8+1, stop };
    }
