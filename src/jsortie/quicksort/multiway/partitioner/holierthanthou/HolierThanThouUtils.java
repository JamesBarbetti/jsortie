...
public class HolierThanThouUtils {
  public static int[] moveEvenlySpacedPivotsToFront
    ( int[] vArray, int[] pivotIndices, int start, int p ) {
    ...
    if ( p == c ) {
      MultiPivotUtils.movePivotsAside(vArray, pivotIndices, correctedPivotIndices);
    } else {
      ...
      for (int w=0; w<c; ++w) {           //w is 0-based
        int vTemp = vArray[start+w];      //item being swapped out
  ...
  public static int[] moveGeometricPivotsToFront(int[] vArray, int[] pivotIndices, int start, int p) {
	int   c = pivotIndices.length; //sampleCount (normally I name this: c)
    int[] correctedPivotIndices = new int [c]; 
    //yes, this might be >desiredPivotCount; it has a *second* purpose.
  ...
  public static int[] selectEvenlySpacedPivots(int[] vArray, int sampleStart, int sampleStop, int p) {
    ...
    if ( p != c ) {
      for (int w=0; w<c; ++w) {   //w is 0-based
        int vTemp = vArray[sampleStart+w];  //item being swapped out
